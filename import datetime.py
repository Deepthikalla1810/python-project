...
# Function to configure scrolling when resizing
def configure_canvas(event):
    canvas.configure(scrollregion=canvas.bbox("all"))

# Function to bind the mouse scroll to the canvas
...
# Pack canvas and scrollbar in the root window
canvas.pack(side=LEFT, fill=BOTH, expand=True)
scrollbar.pack(side=RIGHT, fill=Y)

# Add all the UI elements in the scrollable_frame instead of root

Label(scrollable_frame, text='EXPENSE TRACKER', font=('Noto Sans CJK TC', 20, 'bold'), bg='IndianRed').pack(side=TOP, fill=X, pady=10)
...
# Frames
data_entry_frame = Frame(scrollable_frame, bg='white')
data_entry_frame.place(x=10, y=50, relheight=0.65, relwidth=0.30)

buttons_frame = Frame(scrollable_frame, bg='pink')
...
# Data Entry Frame
Label(data_entry_frame, text='Date (YYYY-MM-DD):', font=('Georgia', 13), bg='white').place(x=10, y=20)
date = DateEntry(data_entry_frame, date=datetime.datetime.now().date(), font=('Times 13 bold'))
date.place(x=10, y=60, width=150)

Label(data_entry_frame, text='Payee:', font=('Georgia', 13), bg='white').place(x=10, y=100)
Entry(data_entry_frame, textvariable=payee, font=('Times 13 bold')).place(x=10, y=130, width=150)

Label(data_entry_frame, text='Description:', font=('Georgia', 13), bg='white').place(x=10, y=170)
Entry(data_entry_frame, textvariable=desc, font=('Times 13 bold')).place(x=10, y=200, width=150)

Label(data_entry_frame, text='Amount:', font=('Georgia', 13), bg='white').place(x=10, y=240)
Entry(data_entry_frame, textvariable=amnt, font=('Times 13 bold')).place(x=10, y=270, width=150)

Label(data_entry_frame, text='Mode of Payment:', font=('Georgia', 13), bg='white').place(x=10, y=310)
OptionMenu(data_entry_frame, MoP, 'Cash', 'Card', 'Online').place(x=10, y=340)

Label(data_entry_frame, text='Category:', font=('Georgia', 13), bg='white').place(x=10, y=380)
